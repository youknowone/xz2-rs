    ...
    if size > 4 as size_t {
        ...
        while buf < limit {
            ...
            crc = lzma_crc64_table[3 as usize][(tmp & 0xff as u32) as usize]
                ^ lzma_crc64_table[2 as usize][(tmp >> 8 as c_int & 0xff as u32) as usize]
                ^ crc >> 32 as c_int
                ^ lzma_crc64_table[1 as usize][(tmp >> 16 as c_int & 0xff as u32) as usize]
                ^ lzma_crc64_table[0 as usize][(tmp >> 24 as c_int) as usize];
        }
    }
    loop {
        let fresh1 = size;
        ...
        let fresh2 = buf;
        buf = buf.offset(1);
        crc = lzma_crc64_table[0 as usize][(*fresh2 as u64 ^ crc & 0xff as u64) as usize]
            ^ crc >> 8 as c_int;
    }
    return !crc;
}
