    ...
    while i <= size {
        if *buffer.offset(i.wrapping_add(1 as size_t) as isize) as ::core::ffi::c_int
            ...
            && *buffer.offset(i.wrapping_add(3 as size_t) as isize) as ::core::ffi::c_int
                & 0xf8 as ::core::ffi::c_int == 0xf8 as ::core::ffi::c_int
        {
            let mut src: uint32_t = (*buffer.offset(i.wrapping_add(1 as size_t) as isize)
                as uint32_t & 7 as uint32_t) << 19 as ::core::ffi::c_int
                ...
                | (*buffer.offset(i.wrapping_add(3 as size_t) as isize) as uint32_t
                    & 7 as uint32_t) << 8 as ::core::ffi::c_int
            ...
            if is_encoder {
                ...
                dest = src
                    .wrapping_sub(
                        now_pos.wrapping_add(i as uint32_t).wrapping_add(4 as uint32_t),
                    );
            }
            dest >>= 1 as ::core::ffi::c_int;
            *buffer.offset(i.wrapping_add(1 as size_t) as isize) = (0xf0 as uint32_t
                | dest >> 19 as ::core::ffi::c_int & 0x7 as uint32_t) as uint8_t;
            *buffer.offset(i.wrapping_add(0 as size_t) as isize) = (dest
                >> 11 as ::core::ffi::c_int) as uint8_t;
            *buffer.offset(i.wrapping_add(3 as size_t) as isize) = (0xf8 as uint32_t
                | dest >> 8 as ::core::ffi::c_int & 0x7 as uint32_t) as uint8_t;
